...
//  };
 ...
 class PolynomialModel {
  public:
   double c_ = 0;  // coefficient of cx^2
   double a_ = 0;  // coefficient of ax
   double b_ = 0;  // intercept b (bias term)
 
   PolynomialModel() = default;
 
   PolynomialModel(double a, double b, double c)
       : a_(a), b_(b), c_(c) {}
 
   explicit PolynomialModel(const PolynomialModel& other)
   ...
   inline int predict(T key) const {
     ...
     return static_cast<int>(c_ * x * x + a_ * x + b_);
   }
 
   inline double predict_double(T key) const {
     double x = static_cast<double>(key);
     return c_ * x * x + a_ * x + b_;
   }
 };
...
//      // NOt enough points, default to y_sum
//      if (count_ <= 1) {
//        model_->a_ = 0;
...
//  };
 ...
 class PolynomialModelBuilder {
  public:
   ...
   explicit PolynomialModelBuilder<T>(PolynomialModel<T>* model) : model_(model) {}
 
   inline void add(T x, int y) {
     count_++;
     long double xd = static_cast<long double>(x);
     long double yd = static_cast<long double>(y);
 
     x_sum_ += xd;
     y_sum_ += yd;
     xx_sum_ += xd * xd;
     xxx_sum_ += xd * xd * xd;
     xxxx_sum_ += xd * xd * xd * xd;
     xy_sum_ += xd * yd;
     x2y_sum_ += xd * xd * yd;
   ...
   void build() {
     if (count_ <= 2) {
       model_->c_ = 0;  // x² term
       model_->a_ = 0;  // x term
       model_->b_ = static_cast<double>(y_sum_) / count_;  // intercept
       return;
     }
 
     long double n = static_cast<long double>(count_);
 
     // Normal equations for quadratic regression
     long double M[3][3] = {
         {n, x_sum_, xx_sum_},
         {x_sum_, xx_sum_, xxx_sum_},
         {xx_sum_, xxx_sum_, xxxx_sum_}
     };
     ...
     // Determinant helper
     auto det = [](long double m[3][3]) -> long double {
     ...
     long double Mb[3][3] = {  // Replace column 0 with RHS (solves for b_)
         {rhs[0], M[0][1], M[0][2]},
         {rhs[1], M[1][1], M[1][2]},
         {rhs[2], M[2][1], M[2][2]}
     };
 
     long double Ma[3][3] = {  // Replace column 1 with RHS (solves for a_)
         {M[0][0], rhs[0], M[0][2]},
         {M[1][0], rhs[1], M[1][2]},
         {M[2][0], rhs[2], M[2][2]}
     };
 
     long double Mc[3][3] = {  // Replace column 2 with RHS (solves for c_)
         {M[0][0], M[0][1], rhs[0]},
         {M[1][0], M[1][1], rhs[1]},
         {M[2][0], M[2][1], rhs[2]}
     };
 
     model_->b_ = static_cast<double>(det(Mb) / D);  // intercept
     model_->a_ = static_cast<double>(det(Ma) / D);  // x term
     model_->c_ = static_cast<double>(det(Mc) / D);  // x² term
   }
  ...
  private:
   ...
   long double xy_sum_ = 0;
   long double x2y_sum_ = 0;  // renamed from xxy_sum_
